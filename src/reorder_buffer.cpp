...
{
    last_rob = 0;
    branch_instr = {{"BEQ",0},{"BNE",1},{"BGTZ",2},{"BLTZ",3},{"BGEZ",4},{"BLEZ",5},{"J",6}};
...
void reorder_buffer::new_rob_head() 
{
    ...
    while(true)
    {
        ...
        else if(rob_buff[0]->instruction.at(0) == 'J')
        {
            hasJump = false;
            cout << "TESTE\n";
            instr_queue_gui.at(rob_buff[0]->instr_pos).text(EXEC,"X");
            ...
            if(instr_type < 2)
                ...
                pred = branch(instr_type,(float)rob_buff[0]->vj);
            if(pred != rob_buff[0]->prediction)//ele entra aqui
            {
                out_iq->write(rob_buff[0]->destination + ' ' + std::to_string(rob_buff[0]->entry));
            ...
            //auto item_proxy = gui_table.at(0).at(rob_buff[0]->entry-1);

            //gui_table.erase(item_proxy);
        }
        else
        {
            wait(SC_ZERO_TIME);
...
bool reorder_buffer::branch(int optype,float value)
{
    switch(optype)
    {
        ...
        case 5:
            ...
            return false;
        case 6:
            //falta validar se colocar um registro maior que o tamanho das instruções ou menor que a primeira instrução
            return true;
        default:
            cerr << "Erro inesperado, abortando..." << endl << flush;
            sc_stop();
